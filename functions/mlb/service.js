...
// NY date offset by n days (n may be negative) — DST-safe (goes via the NY-localized string).
const dayNY = (n) => new Date(Date.now() + n * 24 * 3600 * 1000).toLocaleDateString('en-CA', {timeZone: 'America/New_York'});
...
// When `endDate` is given, fetches the inclusive [date..endDate] range. The Odds API
// surfaces a game's opener as soon as it posts — which can be the evening *before* the
// game's NY date — so snapshot passes fetch a today..tomorrow window to still resolve
// that game's gamePk (D-SITE-008 orphan fix, 2026-07-23). Each returned game keeps its
// own `gameDate`; callers reconcile by team-name pair + first-pitch time.
async function fetchSchedule(date, {lineups = false, endDate = null} = {}) {
  const hydrate = ['probablePitcher', 'team', 'venue', 'linescore', lineups ? 'lineups' : null]
    .filter(Boolean).join(',');
  const range = endDate ? `startDate=${date}&endDate=${endDate}` : `date=${date}`;
...
async function snapshotLinesImpl({trigger = 'cron'} = {}) {
  ...
  // window so a game whose opener the Odds API surfaces the evening before its NY date
  // still resolves to a gamePk instead of falling to the slug fallback (D-SITE-008 orphan fix).
  const date = todayNY();
  let scheduled;
  try {
    scheduled = await fetchSchedule(date, {endDate: dayNY(1)});
  } catch (err) {
 ...
 fetched = await fetchMlbTotals(apiKey);
  ...
  // Index scheduled games by matchup for gamePk + first-pitch reconciliation. Array-valued:
  // doubleheaders list the same team-name pair under two gamePks, which a single-valued map
  // would collapse (orphaning one game). Disambiguated per event by nearest first-pitch time.
  const byPair = {};
 ...
 rejected++; continue;
    ...
    // Resolve the schedule row for this odds event. When a doubleheader yields >1 candidate,
    // pick the one whose first pitch (gameDate) is nearest the odds event's commence time.
    const candidates = byPair[`${c.away}|${c.home}`] || [];
    let sched = null;
    if (candidates.length === 1) {
      sched = candidates[0];
    } else if (candidates.length > 1) {
      const t = c.commenceTime ? new Date(c.commenceTime).getTime() : NaN;
      sched = candidates
        .slice()
        .sort((a, b) => Math.abs(new Date(a.gameDate) - t) - Math.abs(new Date(b.gameDate) - t))[0];
    }
    ...
    // Orphan prevention (D-SITE-008 fix, 2026-07-23): if we cannot resolve a real gamePk,
    // do NOT create a slug-fallback mlb_games doc — such a doc has no gamePk and can never be
    // finalized/graded (the finalizer keys games by gamePk), so it only produces permanent
    // ungradeable orphan noise. Still record the snapshot as a valid line archive, and count
    // the miss in meta for observability. This deliberately drops the slug-id game doc path.
    if (!gamePk) {
      const snapId = `unmatched_${slug(c.away)}-at-${slug(c.home)}_${Math.floor(Date.now() / 60000)}`;
      batch.set(db.collection(collections.snapshots).doc(snapId), {
        tenantId: ARCTURUSDC_TENANT_ID, gamePk: null, gameId: null,
        home: c.home, away: c.away, capturedAt: ts(), capturedAtIso,
        minutesToFirstPitch: minsToFP, trigger, unmatched: true,
        consensus: c.consensus, books: c.books, requestsRemaining: fetched.remaining,
      });
      ops++; unmatched++;
      continue;
    }
    ...
    // Game's own NY date (from its first pitch) — NOT necessarily today's, since the
    // today..tomorrow schedule window can surface tomorrow's opener. Drives the UI's
    // per-date grouping and the finalizer's date query; the doc id stays the gamePk.
    const gameDateNY = firstPitch
      ? new Date(firstPitch).toLocaleDateString('en-CA', {timeZone: 'America/New_York'})
      : date;
    const gamePayload = {
      tenantId: ARCTURUSDC_TENANT_ID, gamePk: gamePk || null, gameId: id,
      date: gameDateNY, home: c.home, away: c.away,
