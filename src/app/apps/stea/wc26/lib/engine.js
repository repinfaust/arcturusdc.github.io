...
export const DEFAULTS = { baseGoals: 1.25, rho: -0.08, maxGoals: 12 };
 ...
 */
export function buildMatch(home, away, opts = {}) {
...
export function marketDoubleChance(m) {
...
export function marketTotals(m, lines = [1.5, 2.5, 3.5]) {
  const out = {};
...
export function marketTeamTotals(m, lines = [0.5, 1.5, 2.5]) {
  const out = {};
  for (const ln of lines) {
...
export function marketAsianHandicap(m, lines = [-2, -1.5, -1, -0.5, 0, 0.5, 1, 1.5, 2]) {
  const out = {};
...
export function marketCorrectScore(m, top = 8) {
  const s = [];
...
export function ev(modelP, offered) { return modelP * (offered - 1) - (1 - modelP); }
export function kelly(modelP, offered, fraction = 0.25) {
 ...
 */
export function recommend(fixtures, ratings, opts = {}) {
...
export function topRecommendation(fixtures, ratings, opts = {}) {
...
export const W_MARKET_1X2 = 0.5;
export const W_MARKET_TOTALS = 0.9;
...
export function noVigBookProb(sel, odds) {
...
export function pricedMatchForFixture(fixture, ratings, opts = {}) {
...
export function recommendCalibrated(fixtures, ratings, opts = {}) {
  const edgeMain = opts.edgeMain ?? 0.03;
  const edgeSmall = opts.edgeSmall ?? 0.015;
  const bankroll = opts.bankroll ?? 100;
  const fair = (p) => (p <= 0 ? Infinity : 1 / p);
...
export function topRecommendationCalibrated(fixtures, ratings, opts = {}) {
 ...
 */
export function gradeHistory(results, ratings, opts = {}) {
