...
export default function ApexTwinDashboard() {
  ...
  return (
    <div className="space-y-8">
      {/* Hero */}
      <div className="apex-panel p-8">
        <h1 className="apex-h1 mb-2">Track Setup Companion</h1>
        <p className="text-apex-soft max-w-2xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Link
          ...
          className="apex-panel p-6 hover:border-apex-mint/50 transition-colors group"
        >
          <div className="flex items-center justify-between mb-3">
            <span className="text-3xl text-apex-mint">+</span>
            <span className="text-apex-soft group-hover:text-apex-mint transition-colors">→</span>
          </div>
          <h3 className="text-apex-white font-semibold mb-1">Log New Session</h3>
          <p className="text-apex-soft text-sm">Record your setup and performance</p>
        </Link>
        ...
        >
          ...
          <h3 className="text-apex-white font-semibold mb-1">My Sessions</h3>
          <p className="text-apex-soft text-sm">Browse your session history</p>
        </Link>
        ...
        >
          ...
          <h3 className="text-apex-white font-semibold mb-1">Paddock View</h3>
          <p className="text-apex-soft text-sm">See what others are running</p>
        </Link>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="apex-panel p-4">
          <div className="apex-label mb-1">Bikes</div>
          <div className="apex-data text-2xl">{loading ? '--' : bikesCount}</div>
        </div>
        <div className="apex-panel p-4">
          <div className="apex-label mb-1">Sessions</div>
          <div className="apex-data text-2xl">{loading ? '--' : recentSessions.length > 0 ? '...' : '0'}</div>
        </div>
        <div className="apex-panel p-4">
          <div className="apex-label mb-1">Last Track</div>
          <div className="apex-data text-lg truncate">
            {loading ? '--' : recentSessions[0]?.trackName || 'None'}
          </div>
        </div>
        <div className="apex-panel p-4">
          <div className="apex-label mb-1">Best Lap</div>
          <div className="apex-data text-2xl text-apex-mint">
      ...
      <div className="apex-panel">
        <div className="flex items-center justify-between p-4 border-b border-apex-stealth">
          <h2 className="apex-h2">Recent Sessions</h2>
          <Link
            href="/apps/stea/apextwin-poc/sessions"
            className="text-apex-mint text-sm hover:text-apex-mint-tint transition-colors"
          >
            View all →
          </Link>
        </div>
        ...
        {loading ? (
          <div className="p-8 text-center text-apex-soft">Loading...</div>
          ...
          <div className="divide-y divide-apex-stealth">
            {recentSessions.map((session) => (
              <Link
                ...
                className="flex items-center justify-between p-4 hover:bg-apex-graphite/50 transition-colors"
              >
      ...
      <div className="apex-panel p-4">
        <div className="flex items-center justify-between">
          ...
          <Link href="/apps/stea/apextwin-poc/bikes" className="apex-btn apex-btn-secondary">
            {bikesCount === 0 ? 'Add Bike' : 'Manage Bikes'}
          </Link>
        </div>
      </div>
    </div>
