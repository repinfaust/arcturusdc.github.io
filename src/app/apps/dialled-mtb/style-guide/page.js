...
const palette = [
  ...
  {
    role: 'Secondary · Functional UI & Brand Background',
    roleColor: COLORS.yellow,
    name: 'Acid Yellow',
    hex: '#E8FF47',
    gradient: 'linear-gradient(135deg, #B8CC00, #E8FF47)',
    desc: 'The action colour. Used on all interactive UI elements such as buttons, due counts, active states, and CTAs.',
...
const usageRules = [
  ...
  ['App icon (D-mark)', COLORS.pink, 'Magenta', 'Anthracite background. Small yellow accent dot in notch corner.'],
  ['Triangle mark', COLORS.pink, 'Magenta', 'Standalone icon use. Loading screen, ride complete badges.'],
  ['Primary buttons (CTAs)', COLORS.yellow, 'Acid Yellow', 'Dark label text on yellow fill. Never white text on yellow.'],
  ['Due task count badges', COLORS.yellow, 'Acid Yellow', 'Replaces red for non-critical due counts.'],
  ['Active tab indicator', COLORS.yellow, 'Acid Yellow', 'Bottom nav active state. Inactive tabs in Slate.'],
...
const donts = [
  ['no', "Don't use yellow on the logo", 'Full yellow wordmark loses the brand/function distinction and clashes with the yellow CTA system.'],
  ['yes', 'Do use yellow for primary interactive actions', 'Primary buttons, CTAs, and active states use acid yellow. Magenta can appear in accents, charts, and loading animations.'],
  ...
  ['no', "Don't use white text on yellow fills", 'Use anthracite (#1A1C1E) as label text on yellow buttons.'],
...
const competitive = [
  ...
  ['Dialled MTB', 'Magenta + Acid Yellow', 'Maintenance tracking — colour space unoccupied'],
];
    ...
    <main className="min-h-screen bg-[#1A1C1E] text-[#E8ECF0] pb-16">
      ...
      <div className="mx-auto max-w-6xl px-6 md:px-8">
        <div className="mt-6 flex flex-wrap items-center gap-3 text-sm">
          <Link href="/apps/dialled-mtb" className="rounded-full border border-[#333840] bg-[#22262A] px-4 py-2 text-[#E8ECF0] hover:border-[#E8FF47] hover:text-[#E8FF47]">
            Back to Dialled MTB
          </Link>
            ...
            {palette.map((item) => (
              <div key={item.name} className="overflow-hidden rounded-2xl border border-[#333840] bg-[#22262A]">
                <div className="h-28 px-5 py-4 flex items-end" style={{ background: item.gradient }}>
                  <span className="font-mono text-xs font-bold tracking-[0.08em]" style={{ color: item.name === 'Acid Yellow' ? COLORS.bg : '#FFFFFF' }}>
                    {item.hex}
                  </span>
                </div>
                <div className="p-5">
        ...
        <Section label="04 · Brand Marks">
          <div className="grid gap-4 md:grid-cols-3">
            <MarkCard
              ...
              desc="The confirmed app icon. Reads as mountain peak, trail sign, and terrain marker. Yellow summit dot bridges the brand and action colours."
            >
              <TriangleMark large />
            </MarkCard>
            <MarkCard
              ...
              desc="Secondary mark using the client's D-form with spine gap and yellow accent dot. Use in widgets and in-app UI, not as the primary app icon."
            >
              <DMark />
            </MarkCard>
            <MarkCard
              title="Wordmark"
              desc="Full magenta wordmark. Dark backgrounds only. The split D/IALLED treatment is retired in favour of a cleaner single-colour read."
            >
        ...
        <Section label="06 · Competitive Context">
          <DataTable
            ...
            rows={competitive.map(([app, color, position]) => [
              ...
              <span key={`${app}-position`} style={app === 'Dialled MTB' ? { color: COLORS.yellow } : { color: COLORS.text }}>{position}</span>,
            ])}
          />
          <p className="mt-4 text-sm leading-6 text-[#6A7680]">
            No major MTB app owns magenta. Combined with the maintenance-focused proposition, this gives Dialled MTB real visual separation.
          </p>
        </Section>

        <Section label="07 · Magenta Confirmed Value">
          <p className="mb-5 text-sm text-[#6A7680]">Brighter value confirmed by client. <code>#F72585</code> is the single source of truth across all assets.</p>
          <DataTable
            ...
            rows={[
              ...
              [
                ...
                <span key="confirmed-char" className="text-[#6A7680]">Electric fuchsia. Stronger contrast at small icon sizes and on OLED.</span>,
                <span key="confirmed-status" className="font-semibold text-[#E8FF47]">Confirmed</span>,
              ],
            ]}
          />
          <p className="mt-4 text-sm leading-6 text-[#6A7680]">
            The confirmed value should be reused consistently across the app icon, wordmark, brand documents, and developer colour tokens.
          </p>
        </Section>
      </div>
      ...
      <footer className="mt-16 border-t border-[#333840] px-6 py-6 md:px-8">
        <div className="mx-auto flex max-w-6xl flex-col gap-3 text-sm text-[#6A7680] md:flex-row md:items-center md:justify-between">
          <p>Dialled MTB Brand Colour System v1.2 · Triangle = primary icon · D-mark = secondary · Anthracite = secondary colour</p>
...
function TriangleMark({ large = false }) {
  return (
    <svg width={large ? 92 : 44} height={large ? 92 : 44} viewBox="0 0 100 100" fill="none" aria-hidden="true">
      ...
      <circle cx="50" cy="12" r="5" fill="#E8FF47" />
    </svg>
  );
}
...
function DMark() {
  return (
    <svg width="92" height="92" viewBox="0 0 100 100" fill="none" aria-hidden="true">
      ...
      <circle cx="19" cy="15" r="5" fill="#E8FF47" />
    </svg>
  );
}
