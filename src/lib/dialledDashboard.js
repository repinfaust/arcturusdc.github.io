...
- ga4.countryActiveUsers: lifetime GA4 active-user observations grouped by country. The client merges duplicate country labels and normalizes the rows into a 100% distribution; it must not present these observations as registered-rider counts. Country is aggregate analytics geography, not a Firestore profile field; anonymous/pre-registration activity is included, the same person may count in more than one country over the period, analytics-consent/thresholding applies, and internal accounts cannot be removed because GA4 is not joined to Firebase users. City is deliberately not queried or reported.
