...
## 2026-07-15/16 — MLB totals investigation (D-SITE-007): definitive negative result
Revived the abandoned Aug-2025 MLB over/under (main total) work using the WC26 anti-fabrication pattern. User goal: correct Over/Under outcome, either/or — NOT value. Full spec: `planning/MLB_TOTALS_MODEL_SPEC.md`.
- **Diagnosis of the 2025 build's death:** data sources were right (MLB Stats API, The Odds API) but the method was the disease — ChatGPT hand-tuned magic constants fit to a 7-game losing streak, "61.5% LOCKED PROFITABLE" over 26 games (coin-flip noise), 100% Under bias, v2→v7c thrash with no forward record. All 2025 model code discarded.
- **Findings (all walk-forward, real data, zero leakage):** team-rate model beats naive league-mean by 0.02 runs MAE (no signal). Season-final pitcher ERA looked like +5.2pts correct-side @ 8.5 — but AS-OF-DATE pitcher form (real game logs, no look-ahead) collapsed it to 50.6% (coin flip): the "signal" was leakage. **Decisive test vs REAL closing line** (Kaggle `oliviersportsdata/mlb-multimarket-sample-2024`, 2,465 real 2024 games, 9-book consensus): model correct-side **49.7%** (break-even 52.4%); baselines always-Over 49.8% / always-Under 50.2% — the line is so well-calibrated even naive one-side bets are coin flips.
- **Open→close movement (real, but unexploitable for us):** "follow the move" @ opener hits 57.7% (real closing-line-value phenomenon) but is hindsight; our model @ opener = 50.0%, move-direction prediction 45.6%. The line is moved by information arriving pre-game (lineups, scratches, sharp money) that season-rate data doesn't contain.
- **VERDICT: no exploitable edge; do NOT build a predictive `/apps/stea/mlb`.** The disciplined negative result IS the deliverable — settled in two afternoons what a live 2025 season learned the hard way. Reusable asset: the WC26 anti-fabrication + as-of-date-no-leakage backtest rig.

## 2026-07-16 — MLB Line-Movement Study collector BUILT (D-SITE-008)
Built the observational collector from `planning/MLB_LINE_STUDY_SPEC.md`. **Research instrument, NOT a betting model** — no predictions, picks, EV claims, or LLM anywhere. Purpose: measure how pre-game information (lineups, scratches, sharp money) moves the MLB total — the horse-racing overnight-odds-vs-SP phenomenon, with timestamps.
- **Code:** `functions/mlb/service.js` (CJS, mirrors `functions/wc26/service.js` — shared `admin`, tenant const, fail-closed, no fabrication), wired in `functions/index.js` with firebase-functions v1 scheduled exports, all `America/New_York` (DST-safe).
- **Schedules:** 7 odds passes/day (`0 9,15,17,23` non-close + `30 12,21` & `15 18` close, close passes survive budget throttle); free MLB event poll `*/30 9-22`; finalizer `30 3`. Each odds pass = 1 credit (shared `WC26_ODDS_API_KEY`); free MLB-schedule gate spends nothing on off-days; burst snapshots capped 3/day; hard budget guard reserves 50 credits for WC26. ≤310 credits/month worst case.
- **Collections (rules added, client read-only, Functions-write):** `mlb_games`, `mlb_line_snapshots`, `mlb_game_events`, `mlb_meta/collector`. One composite index added (`mlb_line_snapshots`: gameId ASC + capturedAtIso DESC).
- **Verified:** live Odds API path returns real MLB totals; pure fns unit-tested incl. edge cases; caught+fixed a pre-deploy bug where the totals-line range gate (4–18) wrongly filtered decimal odds (~1.9) and nulled consensus prices (split into `median()` + `medianLine()`). `mlb/service.js` lints clean.
- **Post-deploy:** needs a 3-day soak against `mlb_meta/collector` (cadence, burst cap, budget guard, off-day skip, doubleheaders). Analysis scripts at the ~8-week mark are a separate approval.
